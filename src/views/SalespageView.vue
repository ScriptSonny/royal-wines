    ...
    <div class="breadcrumb-container">
      <div class="breadcrumb">
        ...
        <span class="breadcrumb-current">
          {{ searchQuery ? `Zoekresultaten voor: ‘${searchQuery}’` : 'Aanbiedingen' }}
        </span>
      </div>
    </div>
          ...
          <div class="products-container">
            <div class="products-grid">
              <ProductCard v-for="(product, index) in paginatedProducts" :key="index" :title="product.title"
                :price="product.price" :salesPrice="product.salesPrice" :volume="product.volume" :image="product.image" />
            </div>
          </div>
        </div>
...
const allProducts = ref([...dummyProducts].filter(p => p.isSale));

function generateFilterOptions(products = allProducts.value) {
...
const sortedProducts = ref([...allProducts.value]);
...
const filteredProducts = computed(() => {
  return sortedProducts.value.filter((product) => {
    ...
    const effectivePrice = product.salesPrice ?? product.price;
...
const handleSort = (option: string) => {
  const sorted = [...allProducts.value];
  const getEffectivePrice = (product: typeof sorted[0]) =>
